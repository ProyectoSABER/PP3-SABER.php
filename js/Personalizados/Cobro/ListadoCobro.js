  ...
  $.ajax({
    ...
    error: function (error) {
      ...
      if (error.status == 404) {
        ...
        console.log(res);
        $("#table-Socios>tbody").html(
          '<tr class=""><td scope="row" colspan="7" class="table-warnnig"><p class="h4 justify-content-center">No se encontraron Socios</p></td></tr>'
        );
        return "";
      }
    },
  ...
  if (Object.keys(cuotas).length > 0) {
    ...
    cuotas.forEach((cuota) => {
      ...
      template += `<tr >
        ...
        <td>${cuota.VCUOTA}</td>
        <td>${cuota.RECARGO}</td>
        <td>${parseInt(cuota.VCUOTA)+parseInt(cuota.RECARGO)}</td>
        <td><a class="btn btn-outline-light" role="button" onClick="imprimirComprobante(${cuota.idDetalleCuota})"><i class="fas fa-file-pdf"></i></a></td>
      </tr>`;
    });
  ...
  function obtenerDatos() {
    $.ajax({
      ...
      success: function (response) {
        ...
        NombreUsuario = result.usuarioActual[0].USUARIO_NOMBRE;
        ApellidoUsuario = result.usuarioActual[0].USUARIO_APELLIDO;
        TipoUsuario = result.usuarioActual[0].USUARIO_NOMTIPOUSUARIO;
        EmailUsuario = result.usuarioActual[0].USUARIO_EMAIL;
      },
      error: function (error) {
        console.log(error);
      },
            ...
            table: {
              ...
              body: [
                ...
                [
                  {},
                  {},
                  { text: `Responsable: ${ApellidoUsuario} ${NombreUsuario}` },
                ],
                [{}, {}, { text: `Usuario: ${EmailUsuario}` }],
                [{}, {}, {}],
              ],
            },
