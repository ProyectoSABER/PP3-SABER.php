...
function RegistrarLibro($ISBN, $Titulo, $Subtitulo, $Idioma, $NEdicion, $Editorial, $CategoriaLibro, $FechaPublicacion, $CantEjemplar, $Proveedor, $UbicacionEstanteria, $BibliotecarioID, $PConeccionBD ,$urlImg='null')
{


    $sql = "INSERT INTO `libro` (`id_Isbn`, `titulo_libro`, `subtitulo_libro`, `id_idioma`, `numEdicion_libro`, `edit_libro`, `categoria_libro`, `fecPublicacion_libro`, `cantidadStock_libro`,  `prove_libro`, `ubiEstanteria_libro`, `responsableCarga_libro`,`ubi_img`) VALUES ('$ISBN', '$Titulo','$Subtitulo', '$Idioma', '$NEdicion', '$Editorial', '$CategoriaLibro', '$FechaPublicacion', '$CantEjemplar', '$Proveedor', '$UbicacionEstanteria', '$BibliotecarioID','$urlImg')";
...
function conocerLibrosDisponibles($PConeccionBD)
{
    $Lista = array();
    $data = array();
...
INNER JOIN ejemplarlibro eje ON eje.id_Libro = lib.id_Isbn
INNER JOIN estadolibro est ON est.id_EstadoLibro = eje.id_EstadoLibro
WHERE est.id_EstadoLibro = 1 AND eje.estadoRegistro_Ejemplar=true GROUP BY lib.id_Isbn";
    ...
    while ($data = mysqli_fetch_array($rs)) {
        ...
        $Lista[$i]['Libro_CategoriaLibro'] = $data['categoria_cateLibro'];
        $Lista[$i]['Libro_FechaPublicacion'] = $data['fecPublicacion_libro'];
        $Lista[$i]['Libro_Stock'] = $data['Cantidad Ejemplares'];
        $Lista[$i]['Libro_FechaIngreso'] = $data['fechaIng_libro'];
        $Lista[$i]['Libro_Proveedor'] = $data['nom_prove'];
        $Lista[$i]['Libro_UbicacionEstanteria'] = $data['ubiEstanteria_libro'];
        $Lista[$i]['Libro_ResponsableCarga'] = $data['nombre_Persona'] . ' ' . $data['apellido_persona'];
...
function conocerUnLibro($ISBN, $PConeccionBD)
{
    ...
    if (!empty($data)) {
        ...
        $Lista['UrlImg'] = $data['ubi_img'];
    }

    return $Lista;
}
function registrarAutoresEnunlibro($ISBN,$idAutor,$PConeccionBD){
$sql="INSERT INTO `libro_autor`(`id_Isbn`, `id_autor`) VALUES ('$ISBN','$idAutor')";
$consulta = mysqli_query($PConeccionBD, $sql);
